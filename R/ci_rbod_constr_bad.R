ci_rbod_constr_bad <- function (x, indic_col, ngood=1, nbad=1, low_w=0, pref=NULL, M, B) 
{
  x_num = x[, indic_col]
  ...
  for (i in 1:n_unit) {
    ...
    for(k in 1:B)
    {
      draw <- sample(fl, size=M, replace=TRUE)
  ...
  r <- list(ci_rbod_constr_bad_est = eff, ci_rbod_constr_bad_weights = w, 
            ci_rbod_constr_bad_target = target, ci_method = "rbod_constr_bad")
  r$call <- match.call()
  class(r) <- "CI"
  r
}